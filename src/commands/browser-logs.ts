...
export async function browserLogs(
  ...
  const { kept: keptLogs, filtered: nbFiltered } = filterNonblockingBrowserlogs(parsedLogs, nbConfig);
  ...
  if (nbFiltered.length > 0) {
    console.log(chalk.gray(`[已过滤 ${nbFiltered.length} 条已知非阻断日志（优化问题，详见 log 的 nonblocking 字段）]`));
  }
