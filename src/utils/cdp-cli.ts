...
// cdp-cli 调用工具：优先直接执行 submodule/vendor 内的 build/index.js
...
//               ├─> vendor/cdp-cli/build/index.js（发布态，prepare-package 打包）
//               └─> deps/cdp-cli/build/index.js（开发态，npm run setup build）

/**
 * 定位 cdp-cli 入口
 *
 * cdp-cli 的 tsc outDir 是 build/（非 dist/），入口为 build/index.js。
 * 发布态优先 vendor/cdp-cli/build/index.js（prepare-package 打包进发布包），
 * 开发态兜底 deps/cdp-cli/build/index.js（submodule，npm run setup 构建）。
...
export function resolveCdpCliEntry(): string {
  ...
  const vendor = path.resolve(root, 'vendor', 'cdp-cli', 'build', 'index.js');
  if (existsSync(vendor)) {
    return vendor;
  }
  return path.resolve(root, 'deps', 'cdp-cli', 'build', 'index.js');
}
